...
def optimize_chain(chain, unary_cost, pairwise_cost, edge_index,
    ...
    p[:,0] = unary_cost[chain[0],:]
    track[:,0] = -1

    for i in xrange(1, n_nodes):
        p[:,i] = unary_cost[chain[i],:]
...
class Over(object):
    ...
    def _loss_augment_unaries(self, unaries, y, weights):
        for label in xrange(self.n_states):
            mask = y != label
            unary_potentials[mask, label] -= weights[mask]
    ...
    def train(X, Y):
        ...
        for x, y in zip(X, Y):
            _edge_index = {}
            for i, edge in enumerate(self.get_edges(x)):
                _edge_index[edge] = i
        ...
        for iteration in xrange(self.max_iter):
            for k in xrange(len(X)):
                ...
                for i in xrange(len(chains[k])):
                    y_hat[k][i], e = optimize_chain(chains[k][i], lambdas[k][i] + 0.5 * aug_unaries, pairwise, edge_index[k])
            ...
            for k in xrange(len(X)):
                ...
                for i in xrange(len(chains[k])):
                    ...
                    lambdas[k][i] += alpha * (mask - 0.5 * lambda_sum[chains[k][i],:])
