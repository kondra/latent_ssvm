...
#    os.chdir("results/syntetic")
    path = '/home/dmitry/Documents/Thesis/latent_ssvm/results/syntetic/'
    weak_results = np.genfromtxt(path + 'weak_labeled.csv', delimiter=',')
    full_results = np.genfromtxt(path + 'full_labeled.csv', delimiter=',')

    weak_results[0, 2] = 0.5
    full_results[0, 1] = 0.5

    y = np.mean(weak_results[:17, 1:6], axis=0)
    y_min = np.min(weak_results[:17, 1:6], axis=0)
    y_max = np.max(weak_results[:17, 1:6], axis=0)
